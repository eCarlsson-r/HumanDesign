  ...
  <div class="p-8 max-w-7xl mx-auto" *ngIf="model()">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold">
        Editing: The {{ model().type }} of {{ model().name }}
      </h1>
    ...
    <div class="bg-white p-6 rounded-xl shadow space-y-6">
      ...
      <div class="grid grid-cols-12 gap-6">
        <div class="col-span-3">
          ...
          <input [(ngModel)]="model().type" class="w-full border rounded px-3 py-2" />
        </div>

        <div class="col-span-5">
          <label class="block text-sm font-medium mb-2">Name</label>
          <input [(ngModel)]="model().name" class="w-full border rounded px-3 py-2" />
        </div>

        <div>
          <label class="block text-sm font-medium mb-2">Gate 1</label>
          <input
            [(ngModel)]="model().cross1"
            class="w-full border rounded px-3 py-2"/>
        </div>

        <div>
          <label class="block text-sm font-medium mb-2">Gate 2</label>
          <input
            [(ngModel)]="model().cross2"
            class="w-full border rounded px-3 py-2"/>
        </div>

        <div>
          <label class="block text-sm font-medium mb-2">Gate 3</label>
          <input
            [(ngModel)]="model().cross3"
            class="w-full border rounded px-3 py-2"/>
        </div>

        <div>
          <label class="block text-sm font-medium mb-2">Gate 4</label>
          <input
            [(ngModel)]="model().cross4"
            class="w-full border rounded px-3 py-2"/>
        </div>
      </div>
...
export default class CmsEditPage {
  public Editor: any;public isBrowser = false;
  ...
  save() {
    ...
    this.api.put(`cms/cross/${this.model().id}`, this.model())
      .subscribe(() => {
        this.saved.set(true);
      });
  }
