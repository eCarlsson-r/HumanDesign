  ...
  <div class="p-8 max-w-7xl mx-auto" *ngIf="model()">
    ...
    <div class="bg-white p-6 rounded-xl shadow space-y-6">
      ...
      <div>
        ...
        <ckeditor *ngIf="Editor" [editor]="Editor" [(ngModel)]="model().preview" />
      </div>

      <div>
        <label class="block font-medium mb-2">Summary</label>
        <ckeditor *ngIf="Editor" [editor]="Editor" [(ngModel)]="model().summary" />
      </div>

      <div>
        <label class="block font-medium mb-2">Detail</label>
        <ckeditor *ngIf="Editor" [editor]="Editor" [(ngModel)]="model().detail" />
      </div>
...
export default class CmsAttributeEditPage {
  public Editor: any;
