  ...
  <div class="p-8 max-w-7xl mx-auto" *ngIf="model()">
    ...
    <div class="bg-white p-6 rounded-xl shadow space-y-6">
      ...
      <div class="grid grid-cols-6 gap-6">
        <div>
          <label class="block text-sm font-medium mb-2">Gate A</label>
          <input
            [(ngModel)]="model().gateA"
        ...
        <div>
          <label class="block text-sm font-medium mb-2">Gate B</label>
          <input
            [(ngModel)]="model().gateB"
            class="w-full border rounded px-3 py-2"/>
        </div>
...
export default class CmsEditPage {
  ...
  save() {
    ...
    this.api.put(`cms/channel/${this.model().id}`, this.model())
      .subscribe(() => {
        this.saved.set(true);
      });
  }
